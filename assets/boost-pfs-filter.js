...
ProductGridItem.prototype.compileTemplate = function(data) {
  ...
  // Add subtitle
  itemHtml = itemHtml.replace(/{{itemSottotitolo}}/g, buildSubtitle(data));
  // Add class Sconto in euro
  itemHtml = itemHtml.replace(/{{itemSconto_in_euro}}/g, buildSconto(data));
...
function buildSconto(data) {
    var html = '';
    var sconto_in_euro = Utils.getProductMetafield(data, 'custom', 'etichetta_sconto_in_euro');
  
    //console.log('Valore di sconto_in_euro:', sconto_in_euro);
  
    if (sconto_in_euro !== null) {   
        html += '<span class="etichetta_sales_in_euro">RISPARMIA _ €</span>';
    }
    return html;
}
...
function buildLabels(data) {
  ...
  if (boostPFSThemeConfig.custom.sale_label_enable && onSale && !soldOut) {
    if(firstVariant && firstVariant.compare_at_price) {
      salePercent = Math.round((firstVariant.compare_at_price - firstVariant.price) * 100 / firstVariant.compare_at_price);    
    ...
    saleLabel = boostPFSTemplate.saleLabelHtml.replace(/{{ savings }}/g, salePercent + '%');
    }
  }
  // Build Labels
...
function buildTagLabels(data, showall) {
  if (boostPFSThemeConfig.custom.tag_label_enable) {
    ...
    // console.log(tagLabel, 'TAG LABEL')
    return tagLabel;
  } else {
    return "";
  }
